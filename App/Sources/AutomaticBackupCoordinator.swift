...
final class AutomaticBackupCoordinator: ObservableObject {
    ...
    static func explainSchedulingError(_ error: Error) -> String {
        ...
        switch code {
        case .unavailable:
            return NSLocalizedString("background tasks are unavailable — Background App Refresh is off for this app or the whole device, or this is the Simulator", comment: "")
        case .tooManyPendingTaskRequests:
            return NSLocalizedString("iOS already holds too many pending requests from this app", comment: "")
        case .notPermitted:
            return NSLocalizedString("this build does not declare the task identifier in its Info.plist, or background activity is turned off for this app", comment: "")
        #if compiler(>=6.2)
        case .immediateRunIneligible:
            // A continued-processing request asked to run now or not at all.
            return NSLocalizedString("iOS is too busy to start it right now", comment: "")
        #endif
        @unknown default:
    ...
    private func backupBlocker(requiringAutomaticBackup: Bool) -> String? {
        if !preferences.completedOnboarding { return NSLocalizedString("Onboarding is not finished", comment: "") }
        if requiringAutomaticBackup, !preferences.automaticBackup { return NSLocalizedString("Automatic Backup is turned off", comment: "") }
        if preferences.selectedAlbumIDs.isEmpty { return NSLocalizedString("No albums are selected", comment: "") }
        if !account.status.isUsable {
            ...
            return UIApplication.shared.isProtectedDataAvailable
                ? NSLocalizedString("No Google account is connected", comment: "")
                : NSLocalizedString("No Google account is available — if the iPhone has not been unlocked since it restarted, the saved account cannot be read yet", comment: "")
        }
        return nil
    }
...
extension AutomaticBackupCoordinator {
    ...
    func refreshContinuedBackup() {
        ...
        if session.isActive {
            ...
            let waiting = queue.rateLimitPauseReason == nil ? nil : NSLocalizedString("Waiting: Google asked the app to slow down", comment: "")
    ...
    private func endContinuedBackup(_ session: ContinuedBackupSession) {
        ...
        let reason = idle ? NSLocalizedString("nothing is left to back up", comment: "") : (queue.pauseReason ?? NSLocalizedString("nothing can move on its own", comment: ""))
